...
export function Heatmap({ estado, semanas = 26 }: { estado: Estado; semanas?: number }) {
  const { columnas, meses } = React.useMemo(() => {
    const hoyISO = hoy();
    // Retrocede hasta el lunes de hace `semanas` semanas.
    const diasAtras = (semanas - 1) * 7 + diaSemanaLunes(hoyISO);
    ...
    return { columnas: cols, meses: etiquetasMes };
  }, [estado, semanas]);

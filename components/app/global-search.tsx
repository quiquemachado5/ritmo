...
import { CalendarDays, Search, Scale, UtensilsCrossed } from "lucide-react";
...
export function GlobalSearch({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const indice = React.useMemo<Resultado[]>(() => {
    const resultados: Resultado[] = NAV_ITEMS.map((item) => ({ id: `ruta:${item.href}`, tipo: "ruta", titulo: item.label, detalle: "Ir a sección", href: item.href, icono: item.icon }));
  ...
  const resultados = React.useMemo(() => {
    ...
    if (!q) return indice.filter((item) => item.tipo === "ruta");
  ...
  function elegir(resultado: Resultado) {
    setConsulta("");
    onOpenChange(false);
    if (resultado.tipo === "ruta") router.push(resultado.href);
  ...
  return (
    <Dialog open={open} onOpenChange={(siguiente) => { if (!siguiente) setConsulta(""); onOpenChange(siguiente); }}>
      <DialogContent className="top-[max(1rem,env(safe-area-inset-top))] flex max-h-[min(82dvh,42rem)] max-w-2xl translate-y-0 flex-col gap-0 overflow-hidden p-0 sm:top-1/2 sm:-translate-y-1/2 sm:max-w-2xl">
        ...
        <div className="flex items-center gap-3 border-b border-border px-4 py-3 sm:px-5">
          ...
          <Input autoFocus value={consulta} onChange={(event) => setConsulta(event.target.value)} placeholder="Buscar comidas, pesajes, notas…" className="h-11 border-0 bg-transparent px-0 text-base shadow-none focus-visible:ring-0" />
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-2 sm:p-3">
          {resultados.length > 0 ? resultados.map((resultado) => (
            <button key={resultado.id} type="button" onClick={() => elegir(resultado)} className="flex min-h-12 w-full items-center gap-3 rounded-xl px-3 py-2 text-left transition-colors hover:bg-secondary focus-visible:bg-secondary focus-visible:outline-none">
              <span className="grid size-9 shrink-0 place-items-center rounded-lg bg-secondary text-muted-foreground"><resultado.icono className="size-4" /></span>

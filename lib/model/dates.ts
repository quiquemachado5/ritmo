/* ============================================================================
   DATES — utilidades de fecha (portado de la app anterior, sin cambios).

   Toda la aplicación identifica un día por su cadena ISO local 'YYYY-MM-DD'.
   Nunca se usa `new Date(iso)` a secas: el parser trata 'YYYY-MM-DD' como UTC y
   desplaza el día en husos negativos.
   ========================================================================= */

const MS_DIA = 86_400_000;

/** Fecha local → 'YYYY-MM-DD'. */
export function aISO(fecha: Date | string | number = new Date()): string {
 ...
 * regresión y los gráficos, para que el eje temporal sea uniforme aunque falten
...
export function limitesMes(clave: string): { desde: string; hasta: string; dias: number } {
...
export function diaSemanaLunes(iso: string): number {

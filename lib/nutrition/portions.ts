...
export function escalarIngrediente(item: ItemNutricional, factor: number): ItemNutricional {
  return { ...escalarNutrientes(item, factor), gramos: item.gramos == null ? undefined : item.gramos * factor,
    cantidad: factor === 1 ? item.cantidad : `${factor.toLocaleString("es-ES", { maximumFractionDigits: 2 })} × (${item.cantidad || "cantidad sin indicar"})` };
}

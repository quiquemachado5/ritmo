...
/** Contador para dar un id único a cada degradado (varios gráficos por página). */
let _gradId = 0;
const nuevoId = (p) => `${p}-${(++_gradId).toString(36)}`;
...
export function graficoPeso(serie, { objetivo = null, alto = 260, color = 'var(--weight)' } = {}) {
  ...
  return `<svg class="chart" viewBox="0 0 ${W} ${H}" preserveAspectRatio="none" role="img"
    ...
    ${rejilla}
    <path class="chart__area--animated" d="${area}" fill="url(#${gid})"/>
    ${suaves.length > 2 ? `<path d="${trazoSuave(suaves)}" fill="none" stroke="var(--ink-4)"
        stroke-width="1.5" stroke-dasharray="4 5" opacity="0.7"/>` : ''}
    <path d="${linea}" fill="none" stroke="${color}" stroke-width="2.5"
      stroke-linecap="round" stroke-linejoin="round"/>
    ${lineaObjetivo}
    ${puntosSvg}
    <circle cx="${ultimo.x.toFixed(1)}" cy="${ultimo.y.toFixed(1)}" r="4.5"
...
export function graficoBalance(serie, { alto = 190 } = {}) {
  ...
  const barras = serie.map((d, i) => {
    ...
    const color = arriba ? 'var(--energy)' : 'var(--weight)';
...
export function anilloComposicion({ grasaPct }, { tam = 190 } = {}) {
  ...
  return `<svg class="ring" viewBox="0 0 ${tam} ${tam}" width="${tam}" height="${tam}" role="img"
    ...
    <g transform="rotate(-90 ${c} ${c})">
      ...
      <circle class="ring__arc" cx="${c}" cy="${c}" r="${rInt}" fill="none" stroke="var(--weight)"
